...
#' @param S_beta Sensitivity matrix for the CLW estimating equations.
#'
...
clw_estimate <- function(Y, Z, w, X, D, S_beta) {
  ...
  pi_sc <- 1 / w
  resid <- (Y * Z) - mu * Z - pi_sc * as.vector(X %*% t(b_vec))
