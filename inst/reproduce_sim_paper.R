...
# se: numeric vector of estimated standard errors across simulation replications,
#
# The function returns:
#   RB(%)      relative bias, computed as:
#              100 * mean((mu_hat - mu_true) / mu_true).
#   v(*100)    average estimated analytic variance, multiplied by 100:
#              mean(se^2) * 100.
#   MSE(*100)  mean squared error, multiplied by 100:
#              mean((mu_hat - mu_true)^2) * 100.
#   SER        ratio of the estimated standard error to the simulated standard error:
#              mean(estimated SE) / simulated SE
#   CP         empirical 95% Wald confidence interval coverage probability

evaluation <- function(mu, se) {
...
table_sp1[1, ] <- evaluation(result_sp1[, 1],  result_sp1[, 2])
table_sp1[2, ] <- evaluation(result_sp1[, 3],  result_sp1[, 4])
table_sp1[3, ] <- evaluation(result_sp1[, 5],  result_sp1[, 6])
table_sp1[4, ] <- evaluation(result_sp1[, 7],  result_sp1[, 8])
table_sp1[5, ] <- evaluation(result_sp1[, 9],  result_sp1[, 10])

kable(round(table_sp1, 2), align = "c")
...
table_sp2[1, ] <- evaluation(result_sp2[, 1],  result_sp2[, 2])
table_sp2[2, ] <- evaluation(result_sp2[, 3],  result_sp2[, 4])
table_sp2[3, ] <- evaluation(result_sp2[, 5],  result_sp2[, 6])
table_sp2[4, ] <- evaluation(result_sp2[, 7],  result_sp2[, 8])
table_sp2[5, ] <- evaluation(result_sp2[, 9],  result_sp2[, 10])

kable(round(table_sp2, 2), align = "c")
...
for (i in seq_len(n_sim)) {
  ...
  # multi-reference with pre-calibration
  fit_true <- est_pw(
  ...
  # multi-reference without pre-calibration
  fit_false <- est_pw(
    data      = list(sc, des_sp1, des_sp2),
    precali   = FALSE,
    p_formula = list(~ x1 + x2, ~ x3 + x4)
...
table_both[1, ] <- evaluation(result_both[, "mean_precali_TRUE"],
                              result_both[, "se_precali_TRUE"])
table_both[2, ] <- evaluation(result_both[, "mean_precali_FALSE"],
                              result_both[, "se_precali_FALSE"])

kable(round(table_both, 2), align = "c")
